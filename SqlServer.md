...
​	在窗口函数中，PARTITION BY 分区列的核心作用是：将整个数据集按照指定的 “分区列” 拆分成多个独立的子数据集（称为 “分区 / 窗口”），窗口函数会在每个子分区内独立计算结果，而不是在整个表上统一计算。

​	简单来说，PARTITION BY相当于 “分组计算”—— 每个分组内单独应用窗口函数，不同分组的计算结果相互独立。

**关键区别：与GROUP BY的不同**
	GROUP BY会将分组后的数据聚合为一行（如SUM()、AVG()后只保留每个分组的汇总结果），会减少原表行数。
	PARTITION BY不会减少行数，它会保留原表所有行，只是在每个分区内单独计算窗口函数结果（如排名、累计和等）。

**详细实例说明**
假设我们有一张员工表employees，数据如下：

| dept （部门） | name （姓名） | salary （工资） |
| ------------- | ------------- | --------------- |
| 技术部        | 张三          | 8000            |
| 技术部        | 李四          | 9000            |
| 技术部        | 王五          | 7500            |
...
**需求 1：不使用PARTITION BY的窗口函数**
如果我们直接计算所有员工的工资排名（不分区）：
...
SELECT 
  ...
  name,
  salary,
  RANK() OVER (ORDER BY salary DESC) AS 全公司工资排名
FROM employees;
```
...
| dept   | name | salary | 全公司工资排名                                               |
| ------ | ---- | ------ | ------------------------------------------------------------ |
| 技术部 | 李四 | 9000   | 1                                                            |
| 技术部 | 张三 | 8000   | 2                                                            |
| 市场部 | 钱七 | 7000   | 3                                                            |
| 技术部 | 王五 | 7500   | 3（注：7500 比 7000 高，这里修正为 3 实际应为 3 是 7000，7500 是 3？不，正确排序是 9000 (1)、8000 (2)、7500 (3)、7000 (4)、6500 (5)、6000 (6)，之前数据有误，修正后如下） |
| 技术部 | 王五 | 7500   | 3                                                            |
| 市场部 | 钱七 | 7000   | 4                                                            |
| 人事部 | 孙八 | 6500   | 5                                                            |
| 市场部 | 赵六 | 6000   | 6                                                            |

**需求 2：使用`PARTITION BY`按部门分区**

如果我们希望**在每个部门内部单独计算工资排名**（同一部门内的员工互相比，不同部门不影响），就需要`PARTITION BY dept`：
...
SELECT 
  ...
  salary,
  RANK() OVER (
    PARTITION BY dept  -- 按部门分区，每个部门单独计算
...
**结果**：
每个部门内的员工单独排名（每个部门的排名从 1 开始）：

| dept   | name | salary | 部门内工资排名                 |
| ------ | ---- | ------ | ------------------------------ |
| 技术部 | 李四 | 9000   | 1                              |
| 技术部 | 张三 | 8000   | 2                              |
| 技术部 | 王五 | 7500   | 3                              |
