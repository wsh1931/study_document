...
​	我是25届毕业生。擅长 Java 后端，也会前端以及小程序的开发，同时了解人工智能相关知识，自己独立开发过项目，也有项目上线的经验。

我最近的一段工作经历是在江西云数医疗管理主要参与过的项目有医疗学术平台，我在里面主要负责会议模块。还有病案质控系统我主要负责的是编写质控逻辑，生成质控结果报告，以及优化质控效率。

另一段实习经历是高达计算机技术有限公司，我参加的项目有物业管理系统我处理的工作主要是定期爬取消防产品并且过滤不必要的产品信息来更新现有的产品信息。以及完成聊天模块。还有团队管理，其中负责的模块是处理成员角色变更的一系列申请审批操作。然后还有安斯泰来医疗管理系统，我负责的部分主要是数据导入，以及项目上线操作。然后是三项安全管理系统，我负责处理的是一系列审批流程。

在本科阶段也参加过算法竞赛以及数学竞赛均有获奖。

## 健帆医生学术平台

是一个基于 **SpringBoot + Vue + Uniapp**　的医疗服务平台

- **创建并进入会议**：对接 **OA 系统**拉取会议信息**，**通过拉取的会议信息对接 **腾讯会议API** 并创建会议。会议申请人可以直接进入会议，并且根据会议的角色不同为其他用户分配**专属二维码**。其他用户通过扫描二维码进入小程序。若用户已经登录，则前往会议签到，签到成功后便可以进入会议。若用户未登录则需要先注册，待注册完成后才能前往会议签到。
- **身份认证并记录会议信息**：用户进入会议之后会通过用户扫描二维码携带的信息根据**腾讯会议事件订阅（Webhook），**判断用户信息是否符合入会要求，不符合则退出会议**。**并且通过 **Webhook** 来更新会议实际开始/结束时间，记录要更新的**云录制视频**信息。
- **推送会议流水至OA 系统**：使用 **Quartz** 定时任务并调用 **腾讯会议API** 拉取会议云录制视频下载地址。将云录制视频下载到本地后推送至 **OA系统** 得到专属文件标识**。**同时使用 **腾讯会议API** 得到参会成员信息并且通过 **itext** 生成**会议签到表**，推送至 **OA系统后** 得到专属文件标识。将得到的专属文件标识、会议信息、参会成员信息等推送至 **OA系统。**

## 苏州市知识产权金融服务平台

基于 **SpringBoot** + **Vue** 的前后端分离项目。

- 使用 **Nginx** + **DockerCompose** 完成项目一键部署上线，提高二次部署的效率。

## 安斯泰来

基于 **SpringCloud** + **Vue** 的微服务前后端分离的医疗服务系统

- 对于读取医生列表 Excel 文件，因为数据量较大（大概 8.6w） 条数据，为了减轻数据库压力、加快读取速度，使用**线程池** + **CountDownLatch** + **EasyExcel** 实现 Excel 数据的导入。
...
* 使用**策略模式** + **工厂模式** + **Spring 配置化**实现多种支付方式：如支付宝沙箱支付等，使用 **Rabbitmq 死信队列**完成当用户在一定时间内未支付更新订单状态，使用 **分布式锁**解决订单限购并保证用户下单的幂等性。使用**线程池**优化下单完成界面的查询速度。
...
- 对于十几个审核流程且项目按审核流程顺序执行，使用**责任链模式**规范审核流程，同时方便扩展新的审核流程。

审核流程：先定义一个 Hander 抽象类，实现 Next（下一个审核流程） 方法，以及一个抽象的 process 审核方法。对于每个审核流程都实现具体的 process  方法，然后导入学员的时候规定审核步骤。

## 物业管理系统

- 发送系统消息时使用 **Redis** 判断用户登录状态，使用 **WebSocket** 完成消息的实时推送，并使用**分布式锁**实现发送消息的幂等性。

  Redis 中存了用户的登录信息，用户每访问一次后端接口都会更新一次在线状态，在 Redis 中查看用户是否登录若已登录则用 WebSocket 发送消息，否则将消息存入数据库中。若用户多次点击，第一次点击请求时该用户会拿到锁，
...
![在这里插入图片描述](images/Java八股文/f090b447b52a6880c81a662f2ec9b066.png)
...
![在这里插入图片描述](images/Java八股文/6771b21be3f1d9bde201a305f87c2c0f.png)
...
![image-20250214215111482](images/Java八股文/image-20250214215111482.png)
...
![image-20250209185544837](images/Java八股文/image-20250209185544837.png)
...
![image-20250209185619083](images/Java八股文/image-20250209185619083.png)
...
![image-20250209185718311](images/Java八股文/image-20250209185718311.png)
...
![image-20250209213138295](images/Java八股文/image-20250209213138295.png)
...
![image-20250209213443431](images/Java八股文/image-20250209213443431.png)
...
![image-20250209215227526](images/Java八股文/image-20250209215227526.png)
...
![image-20250209220403294](images/Java八股文/image-20250209220403294.png)

**源码分析**

![image-20250209220427652](images/Java八股文/image-20250209220427652.png)
...
![image-20250209220459335](images/Java八股文/image-20250209220459335.png)
...
![image-20250209223142270](images/Java八股文/image-20250209223142270.png)
...
![image-20250214222505749](images/Java八股文/image-20250214222505749.png)

![image-20250214222517529](images/Java八股文/image-20250214222517529.png)
...
​	有的时候由于网络等原因可能会出现脑裂的情况，就是说，由于 Redis master 节点和 Redis slave 节点和 Sentinel处于不同的网络分区，使得 Sentinel 没有能够心跳感知到 master，所以通过选举的方式提升了一个 slave 为 master，这样就存在了两个master，就像大脑分裂了一样，这样会导致客户端还在 old master 那里写入数据，新节点无法同步数据，当网络恢复后，Sentinel 会将 old master 降为slave，这时再从新 master 同步数据，这会导致old master中的大量数据丢失。
...
​	I/O多路复用是指利用单个线程来同时监听多个Socket，并且在某个Socket可读、可写时得到通知，从而避免无效的等待，充分利用CPU资源。目前的I/O多路复用都是采用的 epoll 模式实现，它会在通知用户进程 Socket 就绪的同时，把已就绪的 Socket 写入用户空间，不需要挨个遍历 Socket 来判断是否就绪，提升了性能。

​	其中Redis的网络模型就是使用I/O多路复用结合事件的处理器来应对多个 Socket 请求，比如，提供了连接应答处理器、命令回复处理器，命令请求处理器；
...
- 使用了模糊查询且`%`号在前面。
- 在索引字段上进行了运算或类型转换。
- 使用了复合索引（组合索引）但在中间使用了范围查询，导致右边的条件索引失效。
...
​	Spring实现事务的本质是利用 AOP 完成的。它对方法前后进行拦截，在执行方法前开启事务，在执行完目标方法后根据执行情况提交或回滚事务。
...
![image-20250207214123555](images/Java八股文/image-20250207214123555.png)
...
![image-20250207214319065](images/Java八股文/image-20250207214319065.png)

三级缓存解决循环依赖

![image-20250207214345734](images/Java八股文/image-20250207214345734.png)

##### SpringMVC的执行流程

![image-20250207221704472](images/Java八股文/image-20250207221704472.png)

![image-20250207221808541](images/Java八股文/image-20250207221808541.png)
...
![image-20250209114302517](images/Java八股文/image-20250209114302517.png)
...
![image-20250209115750281](images/Java八股文/image-20250209115750281.png)
...
- **服务监控**：服务提供者定期向Eupreka发送心跳以报告健康状态；如果Eureka在一定时间内未接收到心跳，将服务实例从Eureka注册中心剔除。
...
![image-20250209180056669](images/Java八股文/image-20250209180056669.png)
...
![image-20250209183859631](images/Java八股文/image-20250209183859631.png)

![image-20250209183534528](images/Java八股文/image-20250209183534528.png)

##### 出现丢数据怎么解决：当主节点更新数据和还没来得及把数据同步到其他镜像节点

使用仲裁队列，主从模式，基于Raft协议实现强一致性数据同步，简化配置，提高数据安全性。

![image-20250209183604386](images/Java八股文/image-20250209183604386.png)
...
![image-20250210143522072](images/Java八股文/image-20250210143522072.png)
...
![image-20250210210004478](images/Java八股文/image-20250210210004478.png)
...
![image-20250210211051631](images/Java八股文/image-20250210211051631.png)
...
![image-20250210213113323](images/Java八股文/image-20250210213113323.png)

###### MarkWord

![image-20250210213144112](images/Java八股文/image-20250210213144112.png)
...
![image-20250210213209057](images/Java八股文/image-20250210213209057.png)
...
![image-20250210213304872](images/Java八股文/image-20250210213304872.png)

2.通过CAS指令将Lock Record的地址存储在对象头的mark word中（数据进行交换），如果对象处于无锁状态则修改成功，代表该线程获得了轻量级锁。

![image-20250210213318029](images/Java八股文/image-20250210213318029.png)

3.如果是当前线程已经持有该锁了，代表这是一次锁重入。设置Lock Record第一部分为null，起到了一个重入计数器的作用。

![image-20250210213331004](images/Java八股文/image-20250210213331004.png)
...
![image-20250210213502452](images/Java八股文/image-20250210213502452.png)

3.如果Lock Record的 Mark Word不为null，则利用CAS指令将对象头的mark word恢复成为无锁状态。如果失败则膨胀为重量级锁。

![image-20250210213516143](images/Java八股文/image-20250210213516143.png)
...
![image-20250210213706274](images/Java八股文/image-20250210213706274.png)

2.通过CAS指令将Lock Record的**线程id**存储在对象头的mark word中，同时也设置偏向锁的标识为101，如果对象处于无锁状态则修改成功，代表该线程获得了偏向锁。

![image-20250210213719062](images/Java八股文/image-20250210213719062.png)

3.如果是当前线程已经持有该锁了，代表这是一次锁重入。设置Lock Record第一部分为null，起到了一个重入计数器的作用。与轻量级锁不同的时，这里不会再次进行cas操作，只是判断对象头中的线程id是否是自己，因为缺少了cas操作，性能相对轻量级锁更好一些

![image-20250210213739412](images/Java八股文/image-20250210213739412.png)
...
![image-20250210220605433](images/Java八股文/image-20250210220605433.png)
...
![image-20250210223030099](images/Java八股文/image-20250210223030099.png)
...
![image-20250210223045066](images/Java八股文/image-20250210223045066.png)

- 线程2操作：V：int a = 100，A：int a = 100，B：修改后的值：int a = 99(a--)
  - 线程2拿A的值与主内存V的值进行比较，判断是否相等(目前不相等，因为线程1已更新V的值99)
  - 不相等，则线程2更新失败

![image-20250210223059498](images/Java八股文/image-20250210223059498.png)

- 自旋锁操作
  - 因为没有加锁，所以线程不会陷入阻塞，效率较高
  - 如果竞争激烈，重试频繁发生，效率会受影响

![image-20250210223114542](images/Java八股文/image-20250210223114542.png)
...
![image-20250210223149497](images/Java八股文/image-20250210223149497.png)
...
![image-20250210223159598](images/Java八股文/image-20250210223159598.png)
...
![image-20250210225403860](images/Java八股文/image-20250210225403860.png)
...
![image-20250210225421600](images/Java八股文/image-20250210225421600.png)

屏障添加的示意图

![image-20250210225433341](images/Java八股文/image-20250210225433341.png)
...
![image-20250210225522259](images/Java八股文/image-20250210225522259.png)

屏障添加的示意图

![image-20250210225533950](images/Java八股文/image-20250210225533950.png)
...
![image-20250211145356115](images/Java八股文/image-20250211145356115.png)
...
![image-20250211145413518](images/Java八股文/image-20250211145413518.png)
...
![image-20250212115140387](images/Java八股文/image-20250212115140387.png)
...
![image-20250212115208089](images/Java八股文/image-20250212115208089.png)
...
![image-20250212115248399](images/Java八股文/image-20250212115248399.png)

**工作流程**

![image-20250212115310793](images/Java八股文/image-20250212115310793.png)
...
![image-20250212124919703](images/Java八股文/image-20250212124919703.png)

第二：使用jstack查看线程运行的情况，下图是截图的关键信息

运行命令：`jstack -l 46032`

![image-20250212124950654](images/Java八股文/image-20250212124950654.png)
...
![image-20250212153405952](images/Java八股文/image-20250212153405952.png)
...
![image-20250212153436088](images/Java八股文/image-20250212153436088.png)
...
![image-20250212153525108](images/Java八股文/image-20250212153525108.png)
...
![image-20250212155033946](images/Java八股文/image-20250212155033946.png)
...
![image-20250212155052896](images/Java八股文/image-20250212155052896.png)
...
![image-20250212155116363](images/Java八股文/image-20250212155116363.png)
...
![image-20250212155145084](images/Java八股文/image-20250212155145084.png)
...
![image-20250212165728827](images/Java八股文/image-20250212165728827.png)
...
![image-20250212165752436](images/Java八股文/image-20250212165752436.png)
...
![image-20250212172106214](images/Java八股文/image-20250212172106214.png)
...
![image-20250212172627138](images/Java八股文/image-20250212172627138.png)
...
![image-20250212174442515](images/Java八股文/image-20250212174442515.png)
...
![image-20250212174509651](images/Java八股文/image-20250212174509651.png)
...
![image-20250212174535214](images/Java八股文/image-20250212174535214.png)
...
![image-20250212174812937](images/Java八股文/image-20250212174812937.png)
...
![image-20250212200759736](images/Java八股文/image-20250212200759736.png)
...
![image-20250212202934577](images/Java八股文/image-20250212202934577.png)
...
![image-20250212203404935](images/Java八股文/image-20250212203404935.png)
...
![image-20250212211629078](images/Java八股文/image-20250212211629078.png)
...
![image-20250212211704015](images/Java八股文/image-20250212211704015.png)
...
![image-20250212211800763](images/Java八股文/image-20250212211800763.png)

**get方法/remove方法**

![image-20250212211812504](images/Java八股文/image-20250212211812504.png)
...
![image-20250212211849482](images/Java八股文/image-20250212211849482.png)

- 弱引用：表示一个对象处于可能有用且非必须的状态。在GC线程扫描内存区域时，一旦发现弱引用，就会回收到弱引用相关联的对象。对于弱引用的回收，无关内存区域是否足够，一旦发现则会被回收

![image-20250212211901972](images/Java八股文/image-20250212211901972.png)

每一个Thread维护一个ThreadLocalMap，在ThreadLocalMap中的Entry对象继承了WeakReference。其中key为使用弱引用的ThreadLocal实例，value为线程变量的副本

![image-20250212211916039](images/Java八股文/image-20250212211916039.png)
...
![image-20250212214853513](images/Java八股文/image-20250212214853513.png)

**JVM由哪些部分组成，运行流程是什么？**

![image-20250212214911414](images/Java八股文/image-20250212214911414.png)
...
![image-20250212215840964](images/Java八股文/image-20250212215840964.png)
...
![image-20250212223636785](images/Java八股文/image-20250212223636785.png)
...
![image-20250212223718552](images/Java八股文/image-20250212223718552.png)
...
![image-20250212224425667](images/Java八股文/image-20250212224425667.png)

**栈内存溢出情况**

- 栈帧过多导致栈内存溢出，典型问题：递归调用

![image-20250212224502101](images/Java八股文/image-20250212224502101.png)
...
![image-20250213105749047](images/Java八股文/image-20250213105749047.png)
...
![img](images/Java八股文/1739415501423-2.png)

**运行时常量池**

常量池是 *.class 文件中的，当该类被加载，它的常量池信息就会放入运行时常量池，并把里面的符号地址变为真实地址

![img](images/Java八股文/1739415501417-1.png)
...
![img](images/Java八股文/1739416885520-7.png)

下图是NIO传输数据的流程，在这个里面主要使用到了一个直接内存，不需要在堆中开辟空间进行数据的拷贝，jvm可以直接操作直接内存，从而使数据读写传输更快。

![img](images/Java八股文/1739416885520-8.png)
...
![img](images/Java八股文/1739418678924-13.png)
...
![img](images/Java八股文/1739418678924-14.png)
...
![image-20250213115542884](images/Java八股文/image-20250213115542884.png)
...
![img](images/Java八股文/1739420552624-19.png)
...
![img](images/Java八股文/1739421803324-22.png)
...
![img](images/Java八股文/1739421803324-23.png)

- 通过类的全名，获取类的二进制数据流。
- 解析类的二进制数据流为方法区内的数据结构（Java类模型） 
- 创建java.lang.Class类的实例，表示该类型。作为方法区这个类的各种数据的访问入口

![img](images/Java八股文/1739421803324-24.png)

2.验证

![img](images/Java八股文/1739421803324-25.png)
...
![img](images/Java八股文/1739421803324-26.png)
...
![img](images/Java八股文/1739421803324-27.png)

4.解析

![img](images/Java八股文/1739421803324-28.png)

**把类中的符号引用转换为直接引用**

比如：方法中调用了其他方法，方法名可以理解为符号引用，而直接引用就是使用指针直接指向方法。

![img](images/Java八股文/1739421803325-29.png)

5.初始化

![img](images/Java八股文/1739421803325-30.png)
...
![img](images/Java八股文/1739421803325-31.png)
...
![img](images/Java八股文/1739437033543-52.png)
...
![img](images/Java八股文/1739437033543-53.png)

```Java
 String demo = null;
```

![img](images/Java八股文/1739437033543-54.png)

当对象间出现了循环引用的话，则引用计数法就会失效

![img](images/Java八股文/1739437033544-55.png)

先执行右侧代码的前4行代码

![img](images/Java八股文/1739437033544-56.png)

目前上方的引用关系和计数都是没问题的，但是，如果代码继续往下执行，如下图

![img](images/Java八股文/1739437033544-57.png)
...
![img](images/Java八股文/1739437033544-58.png)
...
![img](images/Java八股文/1739437686423-73.png)
...
![img](images/Java八股文/1739437686423-74.png)
...
![img](images/Java八股文/1739437686423-75.png)
...
![img](images/Java八股文/1739438368660-82.png)
...
![img](images/Java八股文/1739438368660-83.png)

- 新创建的对象，都会先分配到eden区

![img](images/Java八股文/1739438368660-84.png)

- 当伊甸园内存不足，标记伊甸园与 from（现阶段没有）的存活对象
- 将存活对象采用复制算法复制到 to 中，复制完毕后，伊甸园和 from 内存都得到释放

![img](images/Java八股文/1739438368660-85.png)

- 经过一段时间后伊甸园的内存又出现不足，标记eden区域to区存活的对象，将存活的对象复制到from区

![img](images/Java八股文/1739438368661-86.png)

![img](images/Java八股文/1739438368661-87.png)

- 当幸存区对象熬过几次回收（最多15次），晋升到老年代（幸存区内存不足或大对象会导致提前晋升）

**MinorGC、 Mixed GC 、 FullGC的区别是什么**

![img](images/Java八股文/1739438368661-88.png)
...
![img](images/Java八股文/1739439623548-103.png)
...
![img](images/Java八股文/1739439623548-104.png)

**CMS（并发）垃圾收集器**

CMS全称 Concurrent Mark Sweep，是一款并发的、使用标记-清除算法的垃圾回收器，该回收器是针对老年代垃圾回收的，是一款以获取最短回收停顿时间为目标的收集器，停顿时间短，用户体验就好。其最大特点是在进行垃圾回收时，应用仍然能正常运行。

![img](images/Java八股文/1739439623548-105.png)

![img](images/Java八股文/1739439623548-106.png)
...
![img](images/Java八股文/1739441553914-115.png)

**Young Collection(年轻代垃圾回收)**

- 初始时，所有区域都处于空闲状态

![img](images/Java八股文/1739441553914-116.png)

- 创建了一些对象，挑出一些空闲区域作为伊甸园区存储这些对象

![img](images/Java八股文/1739441553914-117.png)

- 当伊甸园需要垃圾回收时，挑出一个空闲区域作为幸存区，用复制算法复制存活对象，需要暂停用户线程

![img](images/Java八股文/1739441553914-118.png)

![img](images/Java八股文/1739441553914-119.png)

- 随着时间流逝，伊甸园的内存又有不足
- 将伊甸园以及之前幸存区中的存活对象，采用复制算法，复制到新的幸存区，其中较老对象晋升至老年代

![img](images/Java八股文/1739441553914-120.png)

![img](images/Java八股文/1739441553914-121.png)

![img](images/Java八股文/1739441553914-122.png)

**Young Collection + Concurrent Mark (年轻代垃圾回收+并发标记)** 

当老年代占用内存超过阈值(默认是45%)后，触发并发标记，这时无需暂停用户线程

![img](images/Java八股文/1739441553914-123.png)

- 并发标记之后，会有重新标记阶段解决漏标问题，此时需要暂停用户线程。
- 这些都完成后就知道了老年代有哪些存活对象，随后进入混合收集阶段。此时不会对所有老年代区域进行回收，而是根据暂停时间目标优先回收价值高（存活对象少）的区域（这也是 Gabage First 名称的由来）。

![img](images/Java八股文/1739441553914-124.png)

**Mixed Collection (混合垃圾回收)** 

复制完成，内存得到释放。进入下一轮的新生代回收、并发标记、混合收集

![img](images/Java八股文/1739441553914-125.png)

其中H叫做巨型对象，如果对象非常大，会开辟一块连续的空间存储巨型对象

![img](images/Java八股文/1739441553914-126.png)
...
![img](images/Java八股文/1739441762630-151.png)
...
![img](images/Java八股文/1739441762630-152.png)
...
![img](images/Java八股文/1739441762630-153.png)

![img](images/Java八股文/1739441762630-154.png)
...
![img](images/Java八股文/1739451034417-179.png)
...
![img](images/Java八股文/1739453503226-182.png)
...
![img](images/Java八股文/1739453503227-183.png)
...
![img](images/Java八股文/1739453942986-188.png)
...
![img](images/Java八股文/1739453942986-189.png)
...
![img](images/Java八股文/1739454591231-194.png)

可以内存、线程、类等信息

![img](images/Java八股文/1739454591231-195.png)
...
![img](images/Java八股文/1739454682142-200.png)

监控程序运行情况

![img](images/Java八股文/1739454682142-201.png)

查看运行中的dump

![img](images/Java八股文/1739454682142-202.png)

查看堆中的信息

![img](images/Java八股文/1739454682142-203.png)
...
![img](images/Java八股文/1739455038991-212.png)
...
![img](images/Java八股文/1739455038991-213.png)

3、通过查看堆信息的情况，可以大概定位内存溢出是哪行代码出了问题

![img](images/Java八股文/1739455038991-214.png)
...
![img](images/Java八股文/1739455508908-221.png)
...
![img](images/Java八股文/1739455508908-222.png)
...
![img](images/Java八股文/1739455508908-223.png)
...
![img](images/Java八股文/1739455508908-224.png)
...
![img](images/Java八股文/1739524107567-2.png)

使用jwt解决单点登录的流程如下：

![img](images/Java八股文/1739524107541-1.png)
...
![img](images/Java八股文/1739524484194-7.png)
...
![img](images/Java八股文/1739524484194-8.png)
...
![img](images/Java八股文/1739525759454-13.png)
...
![img](images/Java八股文/1739525759455-15.png)
...
![img](images/Java八股文/1739526733291-51.png)

**ELK基本架构**

ELK即Elasticsearch、Logstash和Kibana三个开源软件的缩写

![img](images/Java八股文/1739526733291-52.png)
...
![img](images/Java八股文/1739526942121-57.png)

也可以在logback配置文件中设置日志的目录和名字

![img](images/Java八股文/1739526942121-58.png)
...
![img](images/Java八股文/1739527006048-63.png)

1. idea中启动远程debug

![img](images/Java八股文/1739527006048-64.png)
...
- 线上诊断工具Arthas(阿尔萨斯)，项目上线之后监控、排查
  ...
  - 核心功能：
  - ![img](images/Java八股文/1739527188776-69.png)

![img](images/Java八股文/1739527188776-70.png)
...
![image-20250218220711743](images/Java八股文/image-20250218220711743.png)
...
#### 在开发中遇到哪些棘手的问题，你是如何解决的

​	在我负责物业管理系统开发时，需要实现业主与物业的实时在线聊天功能，当时遇到了三个非常棘手的线上稳定性问题：一是 WebSocket 长连接在用户断网、页面刷新时，频繁出现消息丢失；二是用户离线后无法收到消息，上线后也不能自动补发；三是会话长期不关闭，占用大量服务连接资源，导致服务内存持续走高，甚至出现 OOM 风险。

我先逐个拆解了问题根源：

​	第一，消息丢失是因为原本直接通过 WebSocket 长连接推送消息，没有持久化和 ACK 确认机制，一旦连接断开，消息发出就直接丢失，没有重试机制；

​	第二，离线消息无法推送，是因为没有做消息的离线存储，用户不在线时消息直接被丢弃；

​	第三，连接资源泄漏，是因为没有做会话超时管理，用户关闭页面后，服务端的连接没有及时释放，长期堆积导致内存占用过高。

针对这些问题，我设计了一套完整的高可用解决方案：

​	消息可靠性保障：引入 RabbitMQ 做消息中转，所有聊天消息先入库持久化，再发送到 MQ，只有收到客户端的 ACK 确认后，才标记消息为已送达，否则通过死信队列做延迟重试，彻底解决了消息丢失问题；

​	离线消息推送：用 Redis 存储用户在线状态，用户不在线时，消息标记为离线状态，通过 XXL-JOB 定时扫描离线消息，用户上线后通过 WebSocket 自动补发所有未读离线消息；

​	会话生命周期管理：基于 Redis+XXL-JOB 做会话心跳检测，用户超过 30 分钟无心跳就自动关闭会话，释放服务端连接资源，解决了连接泄漏的问题。

#### 质控规则举例ss

诊断与年龄是否匹配、诊断与性别是否冲突、中西医病症编码是否符合规范。

#### 扫描二维码进行注册会议签到进入会议到会议结束之后，如何保证导出的成员信息与系统注册的成员信息绑定。

扫码之后进入注册页面，注册完成后跳到会议签到页面，若已经登录则直接跳到会议签到页面，将用户与会议绑定，以后该用户会议管理页面就存在该会议链接，然后进入会议。

#### 如何查找超时接口

​	**根因定位**：业务规则里团队角色层级最多只有 5 层，我编写的成员查找代码，仅设置了「找到目标成员则退出循环」的单一终止条件，没有严格按照业务规则增加「层级超过 5 层强制退出」的兜底终止逻辑。当目标成员不在该团队的层级体系中时，循环无法触发退出条件，最终引发无限循环，持续占用 CPU 资源，导致服务 CPU 打满。

​	**排查过程**：首先通过服务器监控定位到 CPU 使用率 100% 的业务进程，再通过 top -Hp 命令定位到进程内高 CPU 占用的线程，将线程 ID 转为 16 进制后，通过 jstack dump 线程栈，直接定位到了死循环对应的代码行；同时配合平台接口监控，确认该接口的耗时出现了数量级的飙升，和故障时间完全吻合，最终锁定了问题代码。

#### 未来规划

凭借过往的经验快速上手公司的业务与项目，在熟悉公司的业务和项目之后，能主导核心业务的开发，和公司一起做大做强。
...
| -------------- | ---------------------------- | ------------------------------- | -------------------------------- |
| **核心优势**   | 路由灵活、延迟队列成熟       | 高吞吐、分布式事务完善          | 超高吞吐、大数据生态强           |
| **单机 TPS**   | 万级                         | 10 万 +                         | 百万级                           |
| **延迟**       | 微秒级                       | 毫秒级                          | 毫秒级（日志场景可接受）         |
| **分布式事务** | 需额外开发                   | 原生支持                        | 不支持                           |
| **适用场景**   | 订单系统、即时通讯、中小项目 | 金融 / 医疗微服务、大型企业系统 | 日志收集、实时数仓、大数据流处理 |

#### 对于千万级别的数据处理如何保证内存不溢出

分批处理，用CountDownLatch, 初始时设置一个初始值，该值是总数据/ 每次处理的数据量。每执行完数据就使用countdownlanch减一，知道处理完所有数据。
...
#### Redis批量点赞操作，redis里面存的数据是什么

**Key**：`like:unique:{bizType}`

- bizType：业务类型（物业 = property、消防 = fire、公安 = police）

**Field**：`{contentId}:{userId}`

- contentId：被点赞内容 ID（公告 / 资讯 / 工单 ID）
- userId：点赞的业主 ID

#### 项目经理告诉你用户审核完之后要发消息通知管理员，这时候你会和项目经理确认什么需求

**触发时机**：是审核**最终通过**才发，还是通过、驳回都要发；是每一级审核完都发，还是只在终审节点触发。

**接收对象**：是发给全量管理员，还是按物业、消防、公安这类**业务维度**匹配对应管理员。

**通知方式**：用站内信、WebSocket 实时弹框，还是短信推送，是否需要保证消息必达。

**通知内容**：要包含审核单号、申请人、审核结果这些基础信息，是否需要加审批报告的跳转链接。

**异常保障**：发送失败要不要重试、要不要防重复发送，是否需要存通知日志方便后续对账。

**集成方式**：直接在 Flowable 审批节点触发，还是用 RabbitMQ 异步发送，不影响核心审批流程

#### 有两个不同的节点，需要同时拉取数据，如何保证拉取的数据不重复。

**首选：数据分片（无锁、高性能，适合千万级数据）**

​	比如在病案质控系统拉取千万级医疗数据时，按数据的**唯一主键（ID）哈希分片**：给两个节点分配固定分片规则 —— 节点 1 只拉取 ID 为奇数的数据，节点 2 只拉取 ID 为偶数的数据；或者按业务维度（比如物业系统按 “小区 ID” 哈希），每个节点只处理对应小区的工单数据。这种方式无锁竞争，性能最高，完全避免重复。

**兜底：Redis 分布式锁 + 任务队列（高并发场景）**

​	把待拉取数据的 ID 提前放入 Redis List，节点拉取前用 Redisson 加分布式锁，每次从 List 里批量弹出一批 ID（比如 LPOP 1000），锁释放后其他节点再拉取。比如物业系统点赞数据异步落库时，用这个方式保证两个节点拉取的点赞记录不重复。
