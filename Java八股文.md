...
我是来自九江学院软件工程专业的应届本科生。擅长 Java 后端，也会前端以及小程序的开发，自己独立开发过项目，也有项目上线的经验。

我最近的一段实习经历是在高达计算机技术，在其中

​	我主要负责会议模块前后端以及小程序的开发。他的主要流程是先通过对接OA系统来获取会议信息。然后在小程序或者PC端调用腾讯会议API创建会议。会议申请人会发送带有不同身份信息的二维码。用户扫码后先进入签到页面。签到成功后再进入会议。

​	进入会议之前会触发腾讯会议事件订阅来判断该用户是否有资格进入会议。并且使用腾讯会议会议结束和会议开始的事件订阅来更新会议信息。如会议实际开始/结束时间。云录制视频的信息。然后使用Quarzt定时任务定期的爬取会议流水。比如说对于云录制视频要通过腾讯会议API获取其下载地址，下载到本地后上传至OA系统获得OA系统返回的文件标识符。以及将参会成员信息生成PDF后发送至OA系统并返回文件标识符。然后将会议流水推送至OA系统。

然后是威斯特公司实习。

我参与过的项目中其中一个是物业管理平台，基于SpringCloud微服务的前后端分离项目。

使用 **XXL-JOB +** **WebMagic** 爬虫定期爬取消防产品信息查询系统，并使用**布隆过滤器**防止查询到的产品信息重复。

对于用户点赞功能，因为点赞功能比较频繁使用，所以使用 Redis 先暂存点赞，然后使用 XXL-JOB 定时任务定时清理 Redis 的内容，同时因为像园林养护，业主反馈这样的点赞是存入的不同数据库，所以使用线程池来缩短时间。

我先来简单介绍一下 仿 CSDN 项目，这是一个基于SpringCloud的微服务前后端微服务项目。前后端完全由我一人编写。主要包含社区、课程、资源、用户、文章、问答模块。实现了一些比较常用的功能如课程弹幕、用户聊天、支付功能等。
